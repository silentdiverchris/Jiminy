...
## Why
Over the years I've used numerous systems to organise the things I need to do, such as EverNote, OneNote, BoostNote and a few others. 

Essentially I've been trying to shoehorn a personalised version of the GTD system into various hosts that seemed to prefer that I didn't.

While they are all capable, pretty and/or useful in their own ways, I found each to be variously annoying, bloated, awkward or otherwise not ideal, and were tying me into their way of working.

I wanted something that is very simple and quick to add items to, where I can just dump a note, reminder or other to-do item that I need to be nudged about into whatever I'm typing into, then via some unbidden sorcery find them all in one place in a nice structure that helps me prioritise and schedule getting on with them.

I also wanted something that can be backed up easily, where the backups are immediately readable and greppable, and be can be selectively restored or the content copy-pasted; not a proprietary .unreadablebyhumans format that I might find I can't open in a few years without installing something I stopped using ages ago.

Since I use MarkDown for making notes and drafting documents anyway, and a lot of my to-dos in other systems just referenced those .md files (or worse, duplicated chunks of them), I figured why not just add the to-do items (I'll just call them 'items' hereafter) directly into the MarkDown in such a way that they can be easily recognised and dug out by software.
...
This time though, with a system that gathers all the items into one place, reminds me about things and makes it easy to work out what's important while keeping a complete set of everything that I confidently expect to complete during my next two or three lifetimes.
...
This way I have no context switching and no little surprises when Evernote decides to add a new feature I don't want into a well-established work flow, I just keep typing and Jiminy tells me what I need to remember later.
...
The 'FromHere' and 'ToHere' ones are for adding multiple lines to an item, see 'Tagging multiple lines'  below. In human readable form they are '>' and <='.

The tag set above is interpreted as;
...
```
 =b:n-docu=>
 This line will be the item text
 As will this one
 And this too
 <=
```

The delimiters to start and end multi-line tags are under TagSettings, 'FromHere' and 'ToHere'.

Another way to do mutiple lines, especially for when you have a few similar ones to do is to set a context and define a series of items;
...
...
 ...
 =>
Blah multi-line enhancement item..
...
### Enhancements
=ctx-pri:low-prj:Jiminy-enh-b:eve=
=> 
